...
| Closed | Outside the Bar Settings hours, on the bar's clock (default 02:00–08:30, 10:00 Sun) — `bar-clock.js`, 20 Sep 2026 | Chili | "We'll pick up from {opens}." — the next opening from Bar Settings: 8:30, Sundays 10 |
...
### Next session — recorded 19 September 2026
1. **Layout issues seen in real browsers on the test site.** Not listed yet — the author brings the list (page, device + browser, width, screenshot). Reproduce locally at the same width, fix, `npm run build`, push `wip`, `~/deploy-test.sh` on the server, re-check on the device. `website-brief.md` → Platform → Test site → *First findings from real browsers*.
2. **Missing component states.** ~~First: the home hero closed state~~ — built 20 Sep 2026 (entry at the top). Then sweep the other components for states that were never drawn or built — list them before building any.
3. **Test site, remaining setup:** seed Soups **in both stores** (`WP_ROOT=~/SweetPepper-test/public_html php ~/repository-test/tools/menu-seed.php soups`, then the same with `--dishes`; compare at `/menu/#soups` and `/menu/?menu_store=dishes#soups` — PHP CLI is 8.2.32, fine), remove the dead theme link, delete the four installer plugins, SCF *Sync available* + re-enter field values, permalinks → Post name, **basic auth + discourage search engines before the URL is shared** (the site is public until then), Editor accounts for testers.
4. **Main site deploy: manual** (decided 19 Sep) — same pull + rsync route from `~/repository` on `main`, by a script run on purpose, never from the cron. Nothing to build until launch.
