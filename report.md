...
First half of the brief's Soups test (`website-brief.md` → Content editing → Open → *Menu storage*; test logged in `testing.md`). The decision stays open until a manager has tried it; the hybrid store (CPT + Relationship lists) is not built.
...
| `cpt.php` | Custom post types and taxonomies: `dish` (unused — removed if the repeater store wins), `news`, and **`menu_section`** (18 Sep 2026 — one record per menu section, not public, revisions on, admin-only creation; listed in menu order) |
...
2. WordPress editability for dish rows — SCF (replaces ACF, 18 Sep 2026); structure narrowed to repeater-with-row-IDs, `dish` CPT, or CPT + Relationship lists — see `website-brief.md` → Content editing → Open → *Menu storage*. The About hero headline field group is the first working example. **Repeater store built for Soups (18 Sep 2026), as the first half of the brief's Soups test:**
   ...
   - Not built yet: hybrid store B for the comparison; the other 14 sections; dish photo field; the reference dropdown for highlights / previews / picker; section-level content (hero photo, pill, eyebrow, deal card).
