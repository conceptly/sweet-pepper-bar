...
by sweet-pepper-theme/inc/about-data.php, inc/location.php, inc/menu-page.php, inc/home-data.php and tools/page-seed.php.
"""
...
events_home = [
    ...
    *section_header(h, "events", "home_events"),
    hhint("event_cards", "До пяти карточек, порядок строк — порядок на сайте: то, что уже выложено в соцсети. Фото вертикальное 4:5. "
                         "Дата — необязательна: сегодняшняя показывается как «Сегодня!». «Закрепить» выделяет карточку рамкой."),
    h("event_cards", "Карточки", "home_event_cards", "repeater", layout="block", pagination=0, min=0, max=5,
      collapsed=f"{H}event_title_ru", button_label="Добавить карточку", rows_per_page=20, sub_fields=[
          ec("event_cover", "Фото", "cover", "image", 25, instructions="Вертикальное, 4:5.", **image),
          ec("event_title_ru", "Подпись", "title_ru", "text", 25, required=1, **text),
          ec("event_title_en", "Подпись (EN)", "title_en", "text", 25, **text),
          ...
          *twins(ec, "event_alt", "Описание фото (alt)", "alt", placeholder="пусто — как подпись"),
      ]),
...
write_groups((about, location, pairings, menu_food, menu_bar, menu_index, home))