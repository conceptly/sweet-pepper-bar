...
"""Generate the page field groups in sweet-pepper-theme/acf-json/.
  ...
  group_sp_menu.json     — the two menu pages' own words (the door, the Highlights strip, the browser
                           title), on Page Template = Menu OR Menu — Bar (added 24 Sep 2026)
...
by sweet-pepper-theme/inc/about-data.php, inc/location.php, inc/menu-page.php and tools/page-seed.php.
"""
...
# ── The menu pages ── «Меню» (the kitchen, page-menu.php) and «Меню — бар» (page-menu-bar.php):
# one group on both templates, the same three tabs (inc/menu-page.php; typed fallback and
# seed source data/menu/page.php). What a page holds is only what is neither a section's
# («Разделы меню») nor a dish's (the store): the door to the other menu, the Highlights
# header with its list of dishes, the browser title. The connectors, the FOOD / DRINKS
# wordmark and the «Ещё блюда» button stay in code.
M = "field_sp_mpage_"
mp = lambda *args, **k: field(*args, prefix=M, **k)
mtab = lambda key, label: mp(f"{key}_tab", label, "", "tab", placement="top", endpoint=0)
mhint = lambda key, message: mp(f"{key}_hint", "", "", "message", message=message, new_lines="", esc_html=0)

menu_door = [
    mtab("door", "Первый экран"),
    mhint("door", "Дверь в другое меню — блок со стрелкой под списком разделов: на странице кухни она ведёт в бар, "
                  "на странице бара — на кухню. Слово на двери, а при наведении — фото, его подпись и абзац о том меню."),
    *twins(mp, "door_label", "Слово на двери", "menu_door_label", placeholder="Напитки / Еда"),
    mp("door_photo", "Фото при наведении", "menu_door_photo", "image", 34,
       instructions="Горизонтальное, 3:2 — обрезается по центру.", **image),
...
menu_highlights = [
    ...
    *twins(mp, "highlights_headline_2", "Заголовок · строка 2", "menu_highlights_headline_2", placeholder="можно без второй строки"),
    mhint("highlights", "Карточки полосы «Что попробовать» — блюда и напитки из меню, до восьми; порядок здесь — порядок на сайте. "
                        "Название, фото и ссылка «Смотреть в разделе …» берутся из записи блюда: фото — «Фото блюда» в его карточке, "
                        "без фото карточка показывает фото раздела. Пустой список — на сайте стоят блюда с сезонной меткой "
                        "(на странице бара — напитки)."),
    mp("highlights", "Блюда подборки", "menu_highlights", "relationship", "",
...
menu = group(
    "group_sp_menu", "Меню — страница", menu_door + menu_highlights + menu_seo,
    [{"param": "page_template", "operator": "==", "value": "page-menu.php"},
     {"param": "page_template", "operator": "==", "value": "page-menu-bar.php"}],
    "The two menu pages' own words: the door, the Highlights strip, the browser title. Generated by tools/page-field-groups.py; read by inc/menu-page.php.")
    ...
    "group_sp_menu_index", "Меню — папка", [
        mp("index_hint", "", "", "message", new_lines="", esc_html=0,
           message="У этой страницы нет своих текстов: она — папка для двух меню, а адрес /menu/ ведёт на кухню (/menu/food/). "
                   "Первый экран, подборка и название страницы — на страницах «Меню — кухня» и «Меню — бар» ниже в списке."),
    ],
...
write_groups((about, location, pairings, menu, menu_index))
