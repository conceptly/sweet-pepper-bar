...
function sweet_pepper_menu_sections_typed( $state = 'food' ) {
    // 'image' — the hero photo (3:2); 'focus' — its focal point (CSS object-position). Only the
    // tablet band crops the 3:2 photo (to 21:9, menu-hero.css), keeping 64% of its height, so the
    // value says which 64%: '50% 30%' keeps the top, '50% 70%' the bottom. Chosen per photo, Sep 2026.
    // 'band' — the section's own photo, the 4:1 band over its dish list (was typed into each section
    // template until 24 Sep 2026). Both are the typed fallbacks of the page's photo fields.
    // ── Food sections ──
    ...
    $food_sections = [
        'breakfast'   => [
            ...
            'band'        => 'food/breakfast/pepper-breakfast-2.jpg',
            'focus'       => '50% 60%',
        ...
        'lunch'       => [
            ...
            'band'        => 'food/lunch/bagel-lunch-1.jpg',
            'focus'       => '50% 50%',
        ...
        'hot-dishes'  => [
            ...
            'band'        => 'food/dinner/zharkoe-1.jpg',
            'focus'       => '50% 45%',
        ...
        'desserts'    => [
            ...
            'band'        => 'food/dessert/napoleon-1.jpg',
            'focus'       => '50% 40%',
        ...
        'kids'        => [
            ...
            'band'        => 'food/kids/kids-nuggets-2.jpg',
            'focus'       => '50% 70%',
    ...
    $bar_sections = [
        'infusions'    => [
            ...
            'band'        => 'bar/infusions/infusions-lenya-11-4-1.jpg',
            'focus'       => '50% 40%',
        ...
        'cocktails'    => [
            ...
            'band'        => 'bar/cocktails/manhattan-4-1.jpg',
            'focus'       => '50% 40%',
            'caption'     => 'Manhattan',
        ...
        'spirits'      => [
            ...
            'band'        => 'bar/hard-drinks/jim-beam-1.jpg',
            'focus'       => '50% 50%',
        ...
        'no-buzz'      => [
            ...
            'band'        => 'bar/cocktails-non-alco/smoothie-1.jpg',
            'focus'       => '50% 55%',
        ...
        'tea-coffee'   => [
            ...
            'band'        => 'bar/coffee/cappuccino-icecream-1.jpg',
            'focus'       => '50% 50%',
 ...
 * section's own copy (data/menu/sections-copy.php). Keys of the section's fields on its tab of
 * the menu page (`sec_<slug>_{key}_ru` / `_en` — sweet_pepper_menu_section_field()).
 */
 ...
 * page's field is empty, and what tools/page-seed.php menu writes into the fields.
 */
...
/**
 * A section's field name on its menu page: `sec_hot_dishes_headline_ru`,
 * `sec_soups_subsections` (tools/page-field-groups.py → section_tab()).
 *
 * @param string $slug Section slug.
 * @param string $key  Field key without the `sec_<slug>_` prefix.
 */
function sweet_pepper_menu_section_field( $slug, $key ) {
    return 'sec_' . str_replace( '-', '_', $slug ) . '_' . $key;
}

/**
 * A section's field value from its menu page, or '' while the page or the field is missing.
 */
function sweet_pepper_menu_section_value( $slug, $key ) {
    $page = function_exists( 'get_field' ) ? sweet_pepper_menu_page( sweet_pepper_menu_state_for( $slug ) ) : null;
    if ( ! $page ) {
        return '';
    }
    $value = get_field( sweet_pepper_menu_section_field( $slug, $key ), $page->ID );
 ...
 * section, plus headline · eyebrow · pill · alt · deal ( title, main, sub, link ), and the
 * photos — image_url + focus (the hero's), band_image + band_y (the section's band).
 * Per word: the section's field on its menu page in this language → the typed twin in this
 * language → the English as typed. Not a word goes blank while a page is empty.
 *
 * @param string $state 'food' | 'drinks'
...
function sweet_pepper_menu_sections( $state = 'food' ) {
    ...
    foreach ( $sections as $slug => &$sec ) {
        ...
        foreach ( sweet_pepper_menu_section_copy_keys() as $key ) {
            $field       = (string) sweet_pepper_menu_section_value( $slug, "{$key}_{$lang}" );
        ...
        // Photos: the page's, else the typed asset. A slider sets the vertical focus in percent.
        $sec['image_url']  = sweet_pepper_photo_url( sweet_pepper_menu_section_value( $slug, 'hero_photo' ), 'sp-hero', $sec['image'] );
        $focus             = sweet_pepper_menu_section_value( $slug, 'hero_focus' );
        $sec['focus']      = is_numeric( $focus ) ? '50% ' . (int) $focus . '%' : ( $sec['focus'] ?? '50% 50%' );
        $sec['band_image'] = sweet_pepper_photo_url( sweet_pepper_menu_section_value( $slug, 'photo' ), 'sp-4x1', $sec['band'] ?? $sec['image'] );
        $band_y            = sweet_pepper_menu_section_value( $slug, 'photo_y' );
        $sec['band_y']     = is_numeric( $band_y ) ? (int) $band_y : null;
    }
    unset( $sec );
...
/**
 * The band photo's inline style — `--img-y` for reveal.css, which rebuilds the band's
 * object-position from it (plus the scroll drift). Nothing while the slider is untouched.
 *
 * @param array $sec A section of sweet_pepper_menu_sections().
 * @return string ` style="--img-y: 40%"` or ''.
 */
function sweet_pepper_menu_band_style( $sec ) {
    return isset( $sec['band_y'] ) ? ' style="--img-y: ' . (int) $sec['band_y'] . '%"' : '';
}
