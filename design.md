...
- **Dish icons — four, with fixed meanings (author, 18 Sep 2026): leaf = vegetarian · fire = hit · pepper = spicy · Yaroslavl logo = local dish.** Fire is popularity, not heat. Screen readers hear them by these names (Вегетарианское · Хит · Острое · Ярославское блюдо — `dish-row.php`, 25 Sep 2026); every other inline icon is decorative (`aria-hidden`). A dish carries **two at most, never the same one twice**. They render in the order listed — a build default for consistent rows, not an author rule; one line in `tools/menu-field-group.py` changes it. Single-colour like every supporting icon: `.dish-icon` draws them in `currentColor` — Olive by day, Lime by night. Files: `assets/icons/veg|fire|Pepper|yaroslavl-logo.svg`.
