...
7. **Type scale rows** — the HTML scale still lists `Body/Secondary` (Golos 400 · 14) and `Secondary highlight`, which the website has largely replaced with Caption; the team guide already dropped them. **Decided 18 Sep 2026: retired.** The two rows are out of `design.md` §3.3 and the web build carries no 14px text; still to do — remove them from the HTML scale here and delete the Figma styles. The scale also gains a **Tablet** column (Display 64, H1 48). Owner: Etual.
