...
**Found, not changed:** `/menu/` never themes by the hour in the build although the brief's table says it does; the home hero uses the visitor's clock while `visit-hero.js` uses Moscow time. Both are recorded in the brief.
...
### Daypart Time Ranges
*Source of truth: the inline script in `inc/daypart-head.php` (visitor's local clock). The engine reads `data-now`.*
