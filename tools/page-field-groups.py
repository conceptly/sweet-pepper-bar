...
"""Generate the page field groups in sweet-pepper-theme/acf-json/.
  ...
  group_sp_about.json    — the About page's fields: one tab per section, on Page Template = About
                           (a tab per section, all eleven sections since 21 Sep 2026; «Поиск» since 25 Sep)
  group_sp_visit.json    — the Visit page's fields: one tab per section (24 Sep 2026), «Поиск»
                           (25 Sep), on Page Template = Visit
...
# 11. Search — the browser tab and the description search engines show (25 Sep 2026, the home / menu shape).
seo_about = [
    tab("seo", "Поиск"),
    hint("seo", "Название вкладки браузера и строка, которую показывает поиск. Имя сайта добавляется само."),
    *twins(a, "seo_title", "Название страницы", "about_seo_title", placeholder="О нас — гастробар в центре Ярославля"),
    *twins(a, "seo_description", "Описание для поиска", "about_seo_description", "textarea", rows=3, maxlength=160),
]

about = group(
    "group_sp_about", "О баре", hero + concept + reviews + perks + story + guests + team + careers + location_tab + cta + seo_about,
...
visit_seo = [
    vtab("seo", "Поиск"),
    vhint("seo", "Название вкладки браузера и строка, которую показывает поиск. Имя сайта добавляется само."),
    *vt("seo", "title", "Название страницы", placeholder="Как нас найти: адрес и часы работы"),
    *vt("seo", "description", "Описание для поиска", "textarea", rows=3, maxlength=160),
]

visit = group(
    "group_sp_visit", "Как добраться", visit_hero + visit_status + visit_hours + visit_contacts + visit_location + visit_cta + visit_seo,
...
news = group(
    ...
    [
        ...
        n("alt", "Описание фото (alt)", "news_alt", "text", 50, **{**text, "placeholder": "пусто — из медиатеки, иначе как подпись"}),
    ],
    "news",
