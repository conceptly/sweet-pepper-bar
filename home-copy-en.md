# Home — English copy

*Working copy · 16 September 2026 · Not published. Editorial notes and factual checks: [English review](home-copy-review-en.md). Russian companion: [clean draft](home-copy-ru-draft.md). Braces mark unconfirmed data. Service lines are conditional, not confirmed opening hours. Forms, booking explanations, the copyable request and phone-status wording remain deferred and are not replaced by this document.*

## 1. Hero

**Eyebrow:** SHAKE & COOK · YAROSLAVL

| Selection | Headline | Body | Menu button |
|---|---|---|---|
| Breakfast | YUMMY MORNING! | Coffee, eggs and a good reason to get out of bed. | Breakfast menu |
| Lunch | PUMPKIN SOUP TIME! | Soup, something hearty, a little break in your day. | Lunch menu |
| Dinner | READY FOR TONIGHT? | Comfort food, cocktails and a table for your kind of evening. | Dinner menu |
| Party | IT'S COCKTAIL TIME! | Start with your favourite cocktail. See where the evening goes. | Drinks menu |

**Closed hours — built 20 September 2026** (the parked tile carries these instead of its own; the word "closed" stays out of the hero):

| Bar's clock | Parked tile | Headline | Body |
|---|---|---|---|
| 02:00–04:00 | Party | GOOD NIGHT! | See you for breakfast at {8:30 · Sundays 10}. |
| 04:00–08:30 | Breakfast | YOU'RE UP BEFORE THE BAR! | Eggs and coffee from 8:30. |
| Sunday 04:00–10:00 | Breakfast | A LITTLE SUNDAY POLISH | Back at 10, spotless. |

*For the final copy review: consider a shorter "YOU'RE UP BEFORE THE BAR!" — it fits (full column at 1280, two lines on phones) and the author likes it, but it is the longest headline in the set.*

**Primary button — retained:** Reserve

**Current-time marker:** Now

### Conditional service lines — pending schedule confirmation

| State | Copy |
|---|---|
| All-day breakfast available | Breakfast stays all day. |
| Breakfast sparkling offer available | Breakfast stays all day. The Bio Bio Bubbles offer ends at {offer end time}. |
| Weekday lunch available | Weekday lunch till {lunch end time}. |
| Weekend brunch available from the full menu | Weekend brunch — take your pick from the full menu. |
| Kitchen open | The kitchen's on — take your time. |
| Kitchen approaching last orders | Still time to eat — kitchen orders till {last order time}. |
| Venue closed | ~~Closed for now — back at {next opening time}.~~ Superseded for the hero (author, 20 Sep 2026 — no "closed" in the hero): **02:00–04:00** good night — see you for breakfast at {next opening time} · **04:00–08:30** you're up before the bar — eggs and coffee from 8:30 · **Sunday 04:00–10:00** a little Sunday polish — back at 10, spotless. Built split into headline + body — see §1 Hero → Closed hours. |
| Selected offer unavailable | {Offer} returns {next service day/time}. |

## 2. Highlights

**Eyebrow:** A GOOD PLACE TO START

**Headline:** HOUSE SPECIALS / & LOCAL FAVOURITES

**Body:**

A little sweet, a little heat. Start with house infusions, grilled wings or Pepper's pot roast.

**Buttons:** Drinks menu · Food menu

| Card | Description |
|---|---|
| House infusions | From cranberry to raspberry gin. |
| Grilled wings | Honey-glazed wings with sour cream, carrot and celery sticks. |
| Pepper's pot roast | Pork, potato wedges and vegetables in a spicy cream sauce. |

**Infusions category price:** From {minimum price} ₽ · {serving size}

## 3. Bar preview

**Headline:** HOUSE INFUSIONS

| Drink | Description |
|---|---|
| Salted caramel | Sweet, salty, irresistible. |
| Horseradish | For the brave — a taste of Yaroslavl's hot side. |
| Raspberry gin | Gin infused with raspberries. |

**Button:** Explore the drinks menu

**Photo caption — retained:** Community hit!

## 4. Kitchen preview

**Headline:** YOUR LUNCH SORTED

| Dish | Description |
|---|---|
| Pumpkin soup | Creamy pumpkin soup with chicken. |
| Quesadilla | Chicken and cheese, or double cheese. |
| Beef patty bagel | A beef patty, vegetables and pickles in a house-made bagel, with potato wedges and sauce. |

**Button — food-menu destination:** Explore the food menu

## 5. About preview

**Eyebrow:** THE PEPPER STORY

**Headline:** SHAKE & COOK / SINCE {confirmed year}

**Body:**

From Tabasco Bar next door to Sweet Pepper: the same edge, a warmer welcome. A proper meal and a good drink belong at the same table — yours.

**Button:** Read the full story

**Rating:** {verified rating} on Yandex

## 6. Social entrance

**Eyebrow:** MAKE YOUR NEXT PLAN

**Headline:** WHAT'S ON / AT PEPPER

**Body:**

Your next night out starts here. Check VK for news, parties and specials, or take a look on Instagram.

**Buttons:** See what's on VK · View Instagram

**Final link tile:** More on VK

**Post cards:** {supplied post caption} · {actual post date}

## 7. Contacts

**Eyebrow:** YOUR NEXT STOP: KIROVA

**Headline:** SEE YOU SOON?

**Desktop body:**

Drop in, or arrange a table by phone or message. Planning a Friday or Saturday evening? Book ahead.

**Mobile body:**

Drop in, or book ahead for Friday and Saturday evenings.

**Messaging buttons:** Message on VK · Message on Instagram

### Map

**Heading:** Your route to Pepper

**Address:** Kirova 10/25, Yaroslavl

**Actions:** Copy address · Get directions · Yandex Maps

**Copy feedback:** Address copied

### Visit-page link

**Heading:** PLAN YOUR VISIT

**Text:** Hours, parking and the way in.

**Button:** Plan your visit

## 8. Navigation and footer

| Element | Copy |
|---|---|
| Main navigation | Home · Menu · About · Visit |
| Home description | A taste of Sweet Pepper |
| Menu description | From breakfast to late-night drinks |
| About description | The place, the people, the story |
| Visit description | Hours, directions and contacts |
| Social link | What's new |
| Hours heading | Hours |
| Back to top | Back to top |
| Generic copy control / feedback | Copy / Copied |
| Menu controls | Open menu / Close menu |
| Language label | Language |
| Skip link | Skip to content |

**Language switch:** РУС / EN

**Russian-language suggestion on EN:** Удобнее по-русски? Переключить →

**Footer details:**

- {confirmed opening hours}
- Kirova 10/25, Yaroslavl
- {phone number}
- {email address}
- © {year} Sweet Pepper Bar

## 9. Connectors

| Transition | Copy |
|---|---|
| Hero → Highlights | AT SWEET PEPPER |
| Highlights → Bar | FOR A WELL-EARNED POUR |
| Bar → Kitchen | FOR A PROPER APPETITE |
| Kitchen → About | MORE THAN A MENU |
| About → Social | SEE WHAT'S NEW |
| Social → Contacts | JOIN THE PARTY |
