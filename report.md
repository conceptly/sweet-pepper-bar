...
> **Last updated:** 2026-09-23 (EN Location headline IN THE HEART / OF THE CITY, two lines everywhere with a fitted size; Picker opens on the reflection of the word above at every width, bar/ gets its tryTheMatchMaker pair; Bar: Tea & Coffee ends on the picker's word (TRY THE MATCH MAKER / ПАРА ОТ БАРА), bar RU reflection in, лучшийКофеВГороде removed; Menu add-on cards — even 24px inside on all sides; Menu highlights in Russian — headline «Сезонные новинки» alone, card link = section's nav label + arrow, card titles truncate with an ellipsis; Cyrillic headline room — +0.25em under every RU eyebrow for Й / Ё, then 0.15em; menu highlights eyebrow sits on its headline as in the sections; the Russian menu page — section copy on «Разделы меню», bar connectors, hero / highlights / pickers / location / closer, deal cards wrap; menu picker hint — 364px cap off on desktop; menu pickers in Russian — «Спросите Гастробота» + bar / kitchen hints; RU food-menu connectors on /menu/ — eleven words, hero still EN pending a re-export; «Ваш выбор», «ГАСТРОБАРИМ С 2014», «Легенда улицы Кирова»; About RU draft on the site — 17 SCF fields, fallbacks, picker strings; About RU — the author's answers applied: capitals, «Без ступенек» everywhere incl. the page, screen-reader name, ё; Tabasco Bar — ул. Свободы, 9; About RU draft — the author's voice pass and edits reviewed (Concept «МЕСТО, ГДЕ ВСЕ СВОИ», «Спросите Гастробота», perks, founder name confirmed); RU food-menu connectors exported from Figma — 12 words, not yet wired; About story ¶1 and three menu hero lines rewritten in the RU drafts; Food-menu RU connectors — author's editing pass, match-maker line «Пара от бара — вместе вкуснее!»; media topics — «Темы» labels on images; pairings record renamed «Гастробот»; test-site database steps for the day; Menu store decided — Б: all 253 rows in «Блюда» / «Напитки», placed by «Разделы меню»; every section template reads the store; RU twins from menu.md; option А removed from the build and archived in the brief; 2026-09-22: `tools/field-update.php` — field changes for the test site's database; perks stamps — stronger hover, card-recipe pressed; author's RU About copy edits applied to SCF; About hero headline in two fields — stacked two-colour below 992; perks detail row top-aligned on tablets; How it feels — RU cloud words, review links on RU cards, cloud type scales with its column below ~1280, CTA gap 48; About Visit CTA — default RU copy seeded; About hero connector above the fold when logged in; Guests CTA RU → «Фото в VK»; About RU connectors wired — nine RU SVGs, stem map in `template-parts/about/connector.php`; Pairings record («Подбор пары») + Russian UI strings file; About page fully on SCF fields — ten tabs, both languages seeded; Team tab — 6 / 8 members, tenure printed from a year, photos as attachments; Language on the URL built — `/` RU, `/en/` EN, `inc/lang.php`, pill = link; Molot webfont rebuilt with Cyrillic from the OTF; About extraction started — Careers and the shared Location headline read from SCF fields, `sp_field()` / `sweet_pepper_lang()`, Polylang displaced; Home Highlights headline → "& LOCAL HITS" and a phone headline line-count sweep; Test-site round: connector SVGs `preserveAspectRatio="none"` for the WebKit reflection-width fault, drawer descriptions Body 16 from 768, Shake It! 64 in the tablet band, How it feels quote slots + phone cloud unclipped, then the phone commit — chosen word centres, twelve words on phones, field height follows the words, dim 50%; Rail tremble confirmed fixed on the phones, `?debug=` code removed; Rail tremble found — section-level `overflow-x: clip` from the reveal engine, now only on sections with a mask-right element; Rail tremble persists after the split — `?debug=` switches to isolate it on the phone; Rail tremble: real cause found in the author's recording — sticky + scroll container, split in two; iOS phone-number auto-link off; 2026-09-20: Menu storage: А rows compacted to one line, Б size/price in Quick Edit; closed call button Lemon; Menu rail tremble: whole-pixel geometry, to confirm on device; Day reflections at full opacity below 992px; Phone bento: equal row gap whichever tile is lit; night-red options on a contact sheet; Header chip = language switch height; home hero 32px air floor + height-aware tiles; Menu rail: entrance + idle nudge applied, flag removed; Menu rail: 8px below the words as above; Reveal engine: parked headline fix — box-less roots released, `reveal:check` on a section switch; Menu rail: current word = nav word on phones, idle-nudge prototype behind `?rail=nudge`; Nav drawer: descriptions to Caption 13, icon centred on the text; Home contacts: message buttons shortened to the drawers' labels; Bar hours settings: four fields on Bar Settings, code defaults, one source for hero / drawer / Visit / footer; Menu storage: dishes store built for Soups — `dish` posts + `menu_list` Relationship lists, `?menu_store=dishes`; Home hero closed state built — parked lit tile, welcoming copy, `?closed=` override; small hours fixed; Visit hero Sunday rule; One bar clock: home hero, reserve drawer and Visit hero all read Europe/Moscow via `bar-clock.js`; 2026-09-19: Test site test.sweetpepper.bar: theme live from a synced copy — the host refused the symlink; next session recorded under *Next Up* — real-browser layout issues, home hero closed state; Dish picker: Shake It! between the photos, the photos roll, one pairings list for menu + About; 2026-09-18: Menu storage: repeater store built for Soups — `menu_section` records, generated field group, seeder; Platform decisions: SCF, WP Super Cache, Timeweb, menu storage narrowed; Menu section pass + ride suppression + photo-band drift; Menu hero idle sweep + daypart opening frame; Menu hero entrance; Visit hero entrance + scroll-in; dish-picker ticket prints on a pick; About scroll-in + reveal engine reworked to layout triggering; About hero page-load entrance; home motion: page-load entrance, first-paint theme, headline scramble, scroll-in reveals, count-up, photo drift, replay from below; 14px Secondary retired; tablet type mode; home tile radii; connector parallax scoped to heads; icon-beside-text rule)  
> Read this before writing any code. Then read `design.md` and `website-brief.md`.

---

## Menu highlights in Russian — shorter headline and card links — 23 September 2026

Author: the headline "should be just «Сезонные новинки»"; on the cards, truncate the title and drop «Смотреть в разделе», keep the section's name without quotes, shorten a long one, plus the arrow.
- **Headline:** `template-parts/menu-sections/highlights.php` — RU is «Сезонные новинки» on one line; the «Что попробовать» second line (and its night-phone Paprika span) is gone in Russian. EN unchanged.
- **Card link:** RU prints the section's jump-nav `label` (Супы · Салаты · Горячее · Десерты — the «Тексты раздела» field, so the team edits it in one place) at every width, no phone twin. The headline would have been «На первое» / «Салаты от Перцев» / «На горячее» / «На сладкое» — «Салаты от Перцев» overflows a 262px card, and the label matches the chip the guest sees in the rail. EN keeps «Show in Soups» / «In Soups».
- **Titles:** `menu-highlights.css` — the one-line ellipsis moved from the ≤ 991 block to the base title rule, so the desktop rail truncates too (ЛЕТНИЙ САЛАТ С БР…). On 402px phones three RU titles truncate (Летний салат…, Фетучини…, Карамельный…).
- Checked `/menu/` and `/en/menu/` at 1440 and 402, day and night. Rebuilt.

## Location headline: IN THE HEART / OF THE CITY, two lines everywhere — 23 September 2026

Author: drop "very" and "best" "to avoid multiline stacking, especially on mobile"; the headline is IN THE HEART OF THE CITY on two lines. For About / Menu's narrow desktop column: "try a smaller headline … If it doesn't work, we'll try a 2-column headline" (headline above both columns, as Visit).
- **Copy:** `data/location.php` → IN THE HEART / OF THE CITY. Bar Settings saved locally with `tools/field-updates/2026-09-23-location-en.json` (backup first: `mysqldump` to the scratchpad). `tools/field-update.php` now takes `"option": true` for Bar Settings. **Test site: run the same file there** (`WP_ROOT=… php tools/field-update.php tools/field-updates/2026-09-23-location-en.json`) — it applies only while the old EN lines are still in the fields.
- **Layout:** `location.css` — the typed lines break at every width (they flowed as one sentence from 992); `.location__copy` is a size container and `.location__title` takes `min(var(--text-h1), 100cqw / var(--location-fit))`, the lines inherit it; `--location-fit` 6.95 (EN) / 10.05 (`:lang(ru)`). `components.css`: the RU room on Location is `1em` of the fitted size now.
- **Measured (lines / px, About · Menu):** EN 2 lines at every width 1440 → 360 — 64 to ~1215, 59 at 1200, 48 at 1100, 37 at 992; 48 / 36 below. RU 2 lines everywhere — 47 on desktop, 34 at 1100, 25 at 992, 34 / 33 at 375 / 360 (three lines before). Visit unchanged: 64, two lines. Brief → Content editing → About fields → Open holds the numbers and the next step if 992–1100 reads too small. Old wording marked in about-page-copy.md, visit-page-copy-en.md and both copy reviews.

## Picker reflection at every width; the bar's own TRY THE MATCH MAKER — 23 September 2026

Author: "we need a reflection link word in the next section" — the phone-only reflection was an earlier call, reconsidered; the TRY THE MATCH MAKER / ПАРА ОТ БАРА reflections belong in every folder. English follows the Tea & Coffee change (entry below).
- **Both pickers** (`pairing-station.php`, `pairing-station-bar.php`): `.pairing-station__top-word` shows at every width (`menu-section.css` hid it above the phone single-section mode). `dish-picker.css`: `.menu-pairing-station` `padding-top` 80 → 0, so the reflection sits flush under the head word that ends Kids / Tea & Coffee, then the inner 64 to the header — the same 64 as header → bottom word. Phones unchanged (flush, 48).
- **bar/:** `tryTheMatchMaker.svg` + `-reflection.svg` added as copies of the kitchen-night exports (the bar's colours: Lemon line on Peppercorn — `bar/ru/` pair matches kitchen-night/ru too). Tea & Coffee, the bar picker's reflection and the drinks phone tail read `bar/` now; the RU swap maps them to `bar/ru/параОтБараВместеВкуснее(-reflection)`.
- Checked `/menu/`, `/menu/?menu=drinks`, `/en/menu/`, `/en/menu/?menu=drinks` at 1440 (reflection at the section top, 64 to the header) and the RU drinks page at 402 (48).

## Bar page: Tea & Coffee ends on the picker's word — 23 September 2026

Author: added `bar/ru/параОтБараВместеВкуснее-reflection.svg`; the tea/coffee section "should use the same connector since the next section is the picker one", and `лучшийКофеВГороде.svg` was an inconsistent extra.
- `bar/tea-coffee.php`: the bottom word is `kitchen-night/tryTheMatchMaker.svg` in both themes (was `bar/surprisinglyGood.svg`) — the word the bar picker's reflection and the phone tail already showed, so the last section and the picker now pair, as Kids → picker on the food page. (Later the same day: `bar/tryTheMatchMaker.svg`, entry above.) **Both languages:** the English page had the same mismatch (SURPRISINGLY GOOD above a TRY THE MATCH MAKER reflection). `bar/surprisinglyGood.svg` is now unused (kept).
- Russian: `sweet_pepper_menu_connectors_ru()` loses `surprisinglyGood`; the file swap maps the new word to `bar/ru/параОтБараВместеВкуснее.svg` + the new `-reflection` (checked: tea-coffee, picker reflection and phone tail all print the bar/ru pair). `bar/ru/лучшийКофеВГороде.svg` deleted (`git rm`). menu-copy-ru-draft.md bar table updated.

## Menu add-on cards — even room inside — 23 September 2026

Author: inside `.menu-section__addons` "the inner spacing is currently less than the outer spacing"; their trial was card padding 12 / 24, header top 12, row 12 / 16 — "you can try better options".
- **Measured (ink, 1440):** now 20.5 dashed top → title, 11.5 sides, 18.5 last line → scallop tops, title → row 25. The author's trial: 28.5 / 23.5 / 18.5 / 21 — sides fixed, but the top grew past them and the bottom stayed short.
- **Built:** `menu-section.css` — card `padding: 12px 24px 18px` (was 8 12 12), rows inside `padding: 12px 0 16px` (was 16 / 16); header unchanged. Result 24.5 / 24 / 24.5, title → row 21, the same at 402 (the 370 card). All five cards: Любимые соусы ×3, Детские соусы, Растительное молоко. Rebuilt.

## Cyrillic room 0.25 → 0.15em; highlights eyebrow gap as in the menu sections — 23 September 2026

Author, checking in the browser: "set the section headline top padding to 0.15em … it should be enough", and in the highlights "this spacing (brow-headline) seems larger".
- **One value:** `variables.css` → `--cyrillic-room: 0.15` (a number, used as `calc(var(--cyrillic-room) * 1em)`), read by every Cyrillic rule — `components.css` (section-header pairs, heroes, Visit, menu section titles; Location via `× var(--text-h1)`), `about.css` (Story, Guests on phones), `hero.css` (the tiles give back `--text-display × --cyrillic-room`). design.md §3.3 / §9 and styleGuide-report 17 updated (Figma: H1 +10 / +7 / +5, Display +11 / +10).
- **Highlights gap (both languages, ≥ 992):** the shared section-header puts 8px between eyebrow and headline (`.section-title-block` gap 16 − the group's `margin-top: -8px`, the Home / About Figma value); the menu sections stack eyebrow and rail word with none. `menu-highlights.css`: `.menu-highlights .section-headline-group { margin-top: -16px }` — eyebrow top → headline top 24.3px, sections 23.3 (was 32.3). Phones keep the frame's 8 (the sections' eyebrow sits under the rail word there).
- The 1180×690 RU home hero overflow (7px at 0.25) not re-measured — the headline now takes 8px less, so it likely fits; check before deciding a Russian tile floor.

## Cyrillic headline room — Й and Ё under the eyebrow — 23 September 2026

Author, on the Russian menu: "we need a bigger gap between the section headline and the brow because of the й letter" — and, choosing the fix, "for every headline, it should be consistent". Rule in `design.md` → §3.3 *Cyrillic*.
- **Measured:** Molot's breve (Й) and dots (Ё) rise 0.245em above the capitals — 15.7 / 11.8 / 8.8px at 64 / 48 / 36.
- **Built:** `components.css` → *Cyrillic*: `:lang(ru)` + eyebrow-and-headline sibling pairs get `padding-top: 0.25em` (of the headline's own size) — section-header (Home, About, Menu highlights), the menu's phone closer, Home hero, About hero, Visit hero, Visit location; Location (About, Menu) `calc(var(--text-h1) * .25)` because its size lives on the line spans; menu section titles ≥ 992 only (below it the headline is a rail word and the eyebrow sits under it). `about.css`: About's Story and Guests titles flow inline on phones (and Story on tablets) — the room goes on the group there. `hero.css`: in Russian the home tiles give the 0.25 × Display back (`--tile` … `100vh − 612px − 0.25 × --text-display`), so the hero keeps its height where the tile is height-bound. Rebuilt.
- **Verified** (reduced motion, so nothing is parked by the reveal engine — parked, the numbers double, because a headline is parked by its own height): every RU eyebrow → headline gap is English + 16 / 12 / 9px at 1440 / 768 / 402 (+19 for the 76px heroes); within a width every RU section reads the same (20 / 17 / 14); every English gap unchanged (+0). Home and About heroes fit the viewport at 1440×900, 1280×720, 402×874, 375×667 in both languages; at 1440×800 RU and EN heroes are the same height.
- **Open (author):** RU home hero at **1180×690** ends 7px past the viewport (EN 12px inside) — the tile is at its 168 floor there. Options: a Russian floor of 152 (4px grid), or accept it for that size. 1024×650 overflows in both languages already (not new). Figma: the RU frames need the same +0.25em (design.md §9).

## The Russian menu page — 23 September 2026

Author: "check the RU menu copy and bar connectors, document everything and update the website content." Decisions asked and answered: the draft's **### headings** are current (not the heading list / table column); section copy goes into **«Разделы меню» fields**; deal cards, highlights, location and the closer are in this pass.

- **Bar connectors:** the author's eleven exports in `assets/sectionLinks/menu/bar/ru/` (checked by eye, all match their names; #ВПЕППЕРЕПИТЬ keeps its #); `sweet_pepper_menu_connectors_ru( $state )` now has a drinks map and `sweet_pepper_menu_connector_lang()` swaps bar files for `bar/ru/` twins (both day and night paths — one file). Diffed: the RU drinks page changes in exactly 11 connectors; RU food and both EN pages unchanged. Hero DRINKS MENU stays (no export).
- **Section copy → fields.** `tools/menu-field-group.py`: `group_sp_menu_list` gets a collapsed accordion «Тексты раздела» — 12 texts × RU/EN (`sec_label`, `sec_cta_label`, `sec_headline`, `sec_eyebrow`, `sec_description`, `sec_caption`, `sec_pill`, `sec_alt`, `sec_deal_title` / `_main` / `_sub` / `_link`), closed by an endpoint accordion before the dish lists. `data/menu/sections-copy.php` (new): EN eyebrow / headline / pill / deal as typed in the templates + the RU twins of everything, from `menu-copy-ru-draft.md` (### headings, eyebrows, hero texts, nav labels, the offers table); `// draft` marks mine. `inc/menu-sections.php`: `sweet_pepper_menu_sections_typed()` (the English list, renamed), `sweet_pepper_menu_section_copy_keys()`, `sweet_pepper_menu_section_copy_typed()`, `sweet_pepper_menu_sections()` (field → typed twin → English, cached per state + language), `sweet_pepper_menu_section()`. `tools/menu-seed.php all --copy` writes empty fields only (local: 255 written; a second run 0). Consumers switched: 16 section templates (eyebrow, pill, alt, deal card), the rail (headline from data — the three `'headline'` args gone), the hero (labels, captions, texts, buttons).
- **Everything else on the page:** hero strings (`Food` / `Drinks`, `FOOD` / `DRINKS` wordmark, `More plates` / `More pours`, the door intros), the desktop jump-nav's side tab (`Food Menu` / `Drinks Menu` → «Меню кухни» / «Барное меню», mine) and its aria labels, `Menu sections` (the hero nav's was hard-coded), location (`Find the Pepper` → ВАМ СЮДА, entrance alt «Вход в Sweet Pepper» — the About draft keeps «с улицы Кирова» for after the entrance is confirmed, map label; paragraph = About's RU field), the phone closer (headline + body = About's RU fields, eyebrow «ждём в гости» mine, «Забронировать») — `__()` strings in `ru_RU.l10n.php`; highlights (СЕЗОННЫЕ НОВИНКИ + ЧТО ПОПРОБОВАТЬ, dish names from the RU menu, «Смотреть в разделе «На первое»» / «В разделе «…»» by the section's RU headline) and the drinks picker's six rows — typed twins in their templates.
- **English changed on purpose — four photo captions** that named the wrong photo (placeholders): Bar Snacks wings "Pepper's Stuffed Chicken" → Grilled wings; Sandwiches club → Pepper's chicken club; Wine red → Tonight's red; Beer → Ring for a beer (was "Iconic Cobb Salad"). Otherwise EN is byte-identical (diffed; apostrophes now escaped, same text).
- **Deal cards (`menu-section.css`, rebuilt):** the Russian titles overran the fixed 88 title box («игристого», «Идеальная»): `width: min-content; min-width: 88px`. The Russian second lines were cut at the 348 card's edge under `nowrap` (breakfast, wine, the bar-snacks link): the description column and the link may wrap; the main line keeps `nowrap` on desktop (English unchanged — screenshots of all 14 cards compared) and wraps below 992, where the card is the column's width — that also fixes an English overflow at 360 («Drinks deal!», «perfect together», «Spritz Time!»). Measured at 1440 / 1024 / 900 / 768 / 402 / 360, both languages: every card's text inside the card.
- **Checked:** RU food and drinks pages carry no English — text, alt or aria-label — outside the hero connectors (FOOD MENU / DRINKS MENU) and a brand name (Jim Beam White Label); no page overflow at seven widths; RU headlines two lines on desktop for «Для аппетита и на закуску» and «Фирменные настойки» (EN does the same with «Sandwiches & Bagels» at 1024). Local DB backed up before the seed.
- **For the author:** the lines I wrote where the draft has none — `menu-copy-ru-draft.md` → «На сайте»; the wine offer's condition differs between RU and EN; «Детское Меню» typed with a capital М (renders in capitals anyway); the drinks picker's «Jack Daniels on Ice» row shows the Jim Beam photo (pre-existing). **Test site:** SCF → Sync «Раздел меню — списки блюд», then `menu-seed.php all --copy`.

## Bar RU descriptions — draft update, 23 September 2026

Applied the seven proposed bar descriptions to `menu-copy-ru-draft.md` at the author's request for further editing. Aligned «Без алкоголя» in the section heading and heading inventory; tea/coffee eyebrow is now «На чашку и разговор». Kept the optional first-person cocktail line separately labelled for consideration. No website, CMS, connector or SVG changes.

## Menu RU heading alignment and bar review — 23 September 2026

Updated the section headings in `menu-copy-ru-draft.md` to the author's edited food/bar table labels, plus a complete heading inventory and СЕЗОННЫЕ НОВИНКИ. Kept short navigation separate. Author clarified the wine heading as «Винная карта»; applied to both the heading and connector table. Added review notes and wine alternatives without replacing the author's connector copy; corrected the earlier erroneous note that the bar picker starts from food (it starts from drinks). No site/CMS/assets changed.
...
- **Typed fallbacks** (`data/about/*.php`) carry the same values — every old value found once, in one file. **Picker strings** (`languages/ru_RU.l10n.php`, About only at the time — the menu pickers took the same heading later that day, entry above): «Спросите Гастробота», «Выберите блюдо — и получите рекомендацию от бара!»; the shake button's screen-reader name was already «Выбрать случайное блюдо». Not built: the draft's «Фото ВКонтакте» for a standalone button — the page has only the button beside the VK mark («Фото в VK»).
...
| `section-link-word.php` | Full-width SVG text with day/night image swap, optional `class` for reflection modifier. Each `<img>` carries `width`/`height` read from the SVG's viewBox (`sweet_pepper_svg_dimensions()`) so the connector reserves its height before the lazy file loads — without it anchor scrolls on the menu page land short by the height of every connector above the target (fixed 2026-09-10) First use of `foodMenu` / `drinksMenu` and their `-reflection` SVGs: the phone hero connector (Sep 2026). About page: wrapped by `template-parts/about/connector.php` (below) **18 Sep 2026:** optional `loading` arg (`lazy` default, `eager` for a word in the first viewport — the hero's foot word). | **23 Sep 2026:** calls `sweet_pepper_menu_connector_lang()` first — on a Russian food-menu request a kitchen connector swaps to its RU twin (`kitchen-{day,night}/ru/`, both files or none) and takes a Russian alt.
...
| `menu-sections.php` | **Sep 2026.** `sweet_pepper_menu_sections( $state )` — the nine food / seven bar section arrays (label, image, caption, description, nav_variant), moved out of `menu-hero.php` so the hero nav, the jump-nav panel and the phone rail read one list; `sweet_pepper_menu_state_for( $slug )` Each section also carries `cta_label` and `icon` (phone hero commit button), captions and descriptions synced to `menu-hero-copy.md` (Sep 2026) **23 Sep 2026 (the Russian menu):** the English list is now `sweet_pepper_menu_sections_typed()`; `sweet_pepper_menu_sections( $state )` returns the sections in the request's language — each of `sweet_pepper_menu_section_copy_keys()` (label, cta_label, headline, eyebrow, description, caption, pill, alt, deal_*) from the section's «Разделы меню» field → the typed twin (`data/menu/sections-copy.php`, via `sweet_pepper_menu_section_copy_typed()`) → English; `sweet_pepper_menu_section( $slug )` for one. The connector maps have a drinks state (`bar/ru/`). | **23 Sep 2026:** `sweet_pepper_menu_connectors_ru()` (EN stem → RU stem + alt; the hero not yet) and `sweet_pepper_menu_connector_lang()` (the swap, food state only).
...
| `pairing-station.php` | Pairing Station — "Find Your Match!" (food → drink). Tags = food dishes, card = drink pairing. Pairings from `sweet_pepper_food_pairings()` (`inc/pairings.php`, shared with About's Concept since 18 Sep 2026); default pick by slug (`roast`), not by index **23 Sep 2026:** heading and hint are `__()` strings — RU «Спросите Гастробота» / «Выберите блюдо — и получите рекомендацию от бара!». | — | — | YOU'LL LIKE IT |
| `pairing-station-bar.php` | Pairing Station (bar) — tags = drinks, card = food pairing. Swapped data from food version. Always night mode **23 Sep 2026:** heading and hint are `__()` strings — RU «Спросите Гастробота» / «Выберите напиток — и получите рекомендацию от кухни!». | — | — | YOU'LL LIKE IT |
| `location.php` | Entrance image (door-entrance.jpg, 4:1, bottom-12px radius) + Location "Find the Pepper" section (copy left + geo-detected map right). Shared by both food/bar menus Title split into three spans (`line1` / `mid` / `line2`) so the break and colour boundary can move per breakpoint (Sep 2026) | **23 Sep 2026:** subheading, entrance alt and map label are `__()` strings; on a Russian request the paragraph is the About page's «Адрес» field (`sweet_pepper_about_location()`).
| `highlights.php` (menu-sections) | **Sep 2026.** Seasonal Menu Highlights for both menu states (`menu_state` arg): section header, six `menu-highlight-card`s, the "get it while it lasts" connector (`bar/` SVG in the drinks state). Same six kitchen cards in both states for now — the bar's own seasonal cards are the team's job, same data shape; on the drinks page the card links are full URLs to the food page's sections | **23 Sep 2026:** Russian twins in the template — СЕЗОННЫЕ НОВИНКИ + ЧТО ПОПРОБОВАТЬ, eyebrow «вкусно и свежо» (mine), RU dish names, card links «Смотреть в разделе «…»» / «В разделе «…»» by the section's RU headline. **Later on 23 Sep:** headline «Сезонные новинки» alone, card link = the section's nav label (Супы, Салаты…) + arrow at every width.
| `visit-cta.php` (menu-sections) | **Sep 2026, phones only** (Figma Visit CTA 2109:130264): "join the party" eyebrow, "Come sit with us" H1, the About visit copy, one `primary-green` "Reserve a table" button with `c-phone.svg` trailing, `js-reserve-trigger` → reserve sheet. `display: none` above 767px | **23 Sep 2026:** on a Russian request the headline and body are the About page's «Приглашение» fields (`sweet_pepper_about_cta()`); eyebrow and button stay `__()` strings.
...
| `tea-coffee.php` | Tea & Coffee (Coffee top / Tea bottom, dashed olive divider) | Lunch Offer! | Vegan Milk (left col) | TRY THE MATCH MAKER (23 Sep 2026; was SURPRISINGLY GOOD) |
...
0. ~~**Menu storage — test with the team, then decide.**~~ **Decided 23 Sep 2026: Б, whole menu seeded, two lists** — entry at the top. **The Russian menu page:** done 23 Sep (entry at the top) but for the two hero connectors — КУХНЯ ОТ ПЕРЦЕВ (outlined re-export: day, night, reflection, as `кухняОтПерцев.svg`) and БАР ОТ ПЕРЦЕВ (+ reflection; not exported) — then one map line each. Review the lines marked mine (`menu-copy-ru-draft.md` → «На сайте») and the wine offer's condition. **Next on «Разделы меню»:** the section photo field + the vertical-position slider (`--img-y`), now that the section copy lives there. ~~Russian for the menu picker's heading and subtitle~~ — done 23 Sep (the pickers entry). **About story ¶1:** fix the EN twin's facts (ул. Свободы → Кирова), then a field-update file for SCF. Next: first look at «Блюда» / «Напитки» / «Разделы меню» in admin; review the drafted Russian and the RU findings list; the section-level copy (headline, eyebrow, deal card, hero pill) still typed in English in the templates; then the pairings' dish / drink dropdowns (the brief → *Picker pairings* → "When the menu migrates"). *(The stuck-rail tremble: fixed and confirmed on the phones, 21 Sep.)*
