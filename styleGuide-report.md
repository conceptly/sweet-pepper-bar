...
| Figma **Sweet Pepper Style Guide** | file key `Ymf85QTDs0Ttf2rGrx9ter` | Token-driven layout system for the guide: variables → text styles → components → pages. Pages built so far: Cover, Contents, Brand essence (P03), Logo (P04–P07), Colour (P08–P12), Typography (P13–P15). Copy on P02–P12 re-synced to the live `style-guide.html` wording on 2026-09-18. |
...
**Pages built** (📄 Pages, row y = 1300, 655 apart): P01 Cover 27:137, P02 Contents 28:137 (nine sections, 01–09), P03 Brand essence 45:463, P04 Logo family 29:161, P05 hierarchy + do/don't + clear space 33:219, P06 clear-space constructions 37:322, P07 minimum sizes + favicon 39:382, P08 Colour palette & ramps 47:399 (ramp graphic 48:476 — three hue lines from design.md §2.2, mid-chili shown dashed as logo-only), P09 accent palette + neutrals 47:415, P10 imgOverlayDark + Night/Day mode cards 47:431, P11 contrast table + surfaces rule + banner 47:447, P12 semantic-token table + review-by-eye callout 52:512, P13 Typography faces + poster specimen 63:524, P14 display scale (6 Molot rows) 64:542, P15 text scale (7 Golos rows) + responsive callout + type Do/Don't 65:565. The type scale follows `design.md` §3.3 as of 18 Sep (no Secondary rows, Tablet column); samples at ½ px size. Logo Do/Don't now sits on P07 (P05 overflowed after the copy pass; P07's duplicate "backgrounds on screen" callout was removed). Colour took five pages, not three: the overlay callout, the contrast table and the banner example each earn a block. Sample page 7:44 and closing page 11:55 (Lime band, now "09 · Team resources & contact") at y = 0. All footers read "/ 27" (EN PDF page count).
...
7. **Type scale rows — decided 18 Sep 2026 (`design.md` §3.3): Secondary 14 retired, Tablet mode added.** The Figma guide (P14–P15) follows design.md. **Still owed:** the live `style-guide.html` § Typography still shows the two Secondary rows and a desktop→mobile-only responsive callout — edit it in place (§10) and re-render the PDFs; the website Figma file still has the two styles and no Tablet mode (design.md §9).
...
16. **Dish icon legend — added to `design.md` §5.2 on 18 Sep 2026** (leaf = vegetarian · fire = hit · pepper = spicy · Yaroslavl logo = local dish; two per dish at most, never the same twice). The live `style-guide.html` § Icons and the Figma guide's Motifs/Icons pages do not carry it. Decide whether the brand guide needs a menu legend at all, or whether it belongs only on the menu page. Owner: Etual.
17. **Cyrillic headline room — added to `design.md` §3.3 on 23 Sep 2026.** On Russian pages every headline under an eyebrow gets +0.15em above it (0.25 until the author's check, 23 Sep 2026) (room for the breve of Й and the dots of Ё, which rise 0.245em above Molot's capitals). The guide's type section (HTML P14–P15, Figma) should state it next to H1 / Display — RU only, every headline, not just those with Й / Ё.
