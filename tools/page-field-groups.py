...
def section_tab(f, slug, title, prefix, item_type):
    ...
    return [
        ...
        *twins(f, f"{n}eyebrow", "Надзаголовок", f"{n}eyebrow"),
        ...
        f(f"{n}hero_photo", "Фото на первом экране", f"{n}hero_photo", "image", 34, instructions="Горизонтальное, 3:2.", **image),
        f(f"{n}hero_focus", "Что оставить в кадре на планшете", f"{n}hero_focus", "range", 33,
          instructions="Планшет обрезает фото сверху и снизу: 0 — верх, 100 — низ.", **slider),
        f(f"{n}hero_spacer", "", "", "message", 33, message="", new_lines="", esc_html=0),
        ...
        *twins(f, f"{n}description", "Абзац на первом экране", f"{n}description", "textarea", rows=3),
        ...
        f(f"{n}photo", "Фото раздела", f"{n}photo", "image", 34, instructions="Горизонтальное; на сайте — полоса 4:1.", **image),
        f(f"{n}photo_y", "Вертикальное положение", f"{n}photo_y", "range", 33,
          instructions="Какую часть фото оставить в полосе: 0 — верх, 100 — низ.", **slider),
        f(f"{n}band_spacer", "", "", "message", 33, message="", new_lines="", esc_html=0),
...
write_groups((about, location, pairings, menu_food, menu_bar, menu_index))