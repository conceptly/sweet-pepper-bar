...
> **Last updated:** 2026-09-24 (Two menu pages — `/menu/` and `/menu/bar/`, the state is the template, `?menu=drinks` → 301; «Меню — страница» field group on both — door, Highlights as dish references with the seasonal-label fallback, browser title + description; dish photo = the featured image, «Фото» column; 2026-09-23: Cyrillic headline room — +0.25em under every RU eyebrow for Й / Ё; the Russian menu page — section copy on «Разделы меню», bar connectors, hero / highlights / pickers / location / closer, deal cards wrap; menu picker hint — 364px cap off on desktop; menu pickers in Russian — «Спросите Гастробота» + bar / kitchen hints; RU food-menu connectors on /menu/ — eleven words, hero still EN pending a re-export; «Ваш выбор», «ГАСТРОБАРИМ С 2014», «Легенда улицы Кирова»; About RU draft on the site — 17 SCF fields, fallbacks, picker strings; About RU — the author's answers applied: capitals, «Без ступенек» everywhere incl. the page, screen-reader name, ё; Tabasco Bar — ул. Свободы, 9; About RU draft — the author's voice pass and edits reviewed (Concept «МЕСТО, ГДЕ ВСЕ СВОИ», «Спросите Гастробота», perks, founder name confirmed); RU food-menu connectors exported from Figma — 12 words, not yet wired; About story ¶1 and three menu hero lines rewritten in the RU drafts; Food-menu RU connectors — author's editing pass, match-maker line «Пара от бара — вместе вкуснее!»; media topics — «Темы» labels on images; pairings record renamed «Гастробот»; test-site database steps for the day; Menu store decided — Б: all 253 rows in «Блюда» / «Напитки», placed by «Разделы меню»; every section template reads the store; RU twins from menu.md; option А removed from the build and archived in the brief; 2026-09-22: `tools/field-update.php` — field changes for the test site's database; perks stamps — stronger hover, card-recipe pressed; author's RU About copy edits applied to SCF; About hero headline in two fields — stacked two-colour below 992; perks detail row top-aligned on tablets; How it feels — RU cloud words, review links on RU cards, cloud type scales with its column below ~1280, CTA gap 48; About Visit CTA — default RU copy seeded; About hero connector above the fold when logged in; Guests CTA RU → «Фото в VK»; About RU connectors wired — nine RU SVGs, stem map in `template-parts/about/connector.php`; Pairings record («Подбор пары») + Russian UI strings file; About page fully on SCF fields — ten tabs, both languages seeded; Team tab — 6 / 8 members, tenure printed from a year, photos as attachments; Language on the URL built — `/` RU, `/en/` EN, `inc/lang.php`, pill = link; Molot webfont rebuilt with Cyrillic from the OTF; About extraction started — Careers and the shared Location headline read from SCF fields, `sp_field()` / `sweet_pepper_lang()`, Polylang displaced; Home Highlights headline → "& LOCAL HITS" and a phone headline line-count sweep; Test-site round: connector SVGs `preserveAspectRatio="none"` for the WebKit reflection-width fault, drawer descriptions Body 16 from 768, Shake It! 64 in the tablet band, How it feels quote slots + phone cloud unclipped, then the phone commit — chosen word centres, twelve words on phones, field height follows the words, dim 50%; Rail tremble confirmed fixed on the phones, `?debug=` code removed; Rail tremble found — section-level `overflow-x: clip` from the reveal engine, now only on sections with a mask-right element; Rail tremble persists after the split — `?debug=` switches to isolate it on the phone; Rail tremble: real cause found in the author's recording — sticky + scroll container, split in two; iOS phone-number auto-link off; 2026-09-20: Menu storage: А rows compacted to one line, Б size/price in Quick Edit; closed call button Lemon; Menu rail tremble: whole-pixel geometry, to confirm on device; Day reflections at full opacity below 992px; Phone bento: equal row gap whichever tile is lit; night-red options on a contact sheet; Header chip = language switch height; home hero 32px air floor + height-aware tiles; Menu rail: entrance + idle nudge applied, flag removed; Menu rail: 8px below the words as above; Reveal engine: parked headline fix — box-less roots released, `reveal:check` on a section switch; Menu rail: current word = nav word on phones, idle-nudge prototype behind `?rail=nudge`; Nav drawer: descriptions to Caption 13, icon centred on the text; Home contacts: message buttons shortened to the drawers' labels; Bar hours settings: four fields on Bar Settings, code defaults, one source for hero / drawer / Visit / footer; Menu storage: dishes store built for Soups — `dish` posts + `menu_list` Relationship lists, `?menu_store=dishes`; Home hero closed state built — parked lit tile, welcoming copy, `?closed=` override; small hours fixed; Visit hero Sunday rule; One bar clock: home hero, reserve drawer and Visit hero all read Europe/Moscow via `bar-clock.js`; 2026-09-19: Test site test.sweetpepper.bar: theme live from a synced copy — the host refused the symlink; next session recorded under *Next Up* — real-browser layout issues, home hero closed state; Dish picker: Shake It! between the photos, the photos roll, one pairings list for menu + About; 2026-09-18: Menu storage: repeater store built for Soups — `menu_section` records, generated field group, seeder; Platform decisions: SCF, WP Super Cache, Timeweb, menu storage narrowed; Menu section pass + ride suppression + photo-band drift; Menu hero idle sweep + daypart opening frame; Menu hero entrance; Visit hero entrance + scroll-in; dish-picker ticket prints on a pick; About scroll-in + reveal engine reworked to layout triggering; About hero page-load entrance; home motion: page-load entrance, first-paint theme, headline scramble, scroll-in reveals, count-up, photo drift, replay from below; 14px Secondary retired; tablet type mode; home tile radii; connector parallax scoped to heads; icon-beside-text rule)  
> Read this before writing any code. Then read `design.md` and `website-brief.md`.

---

## Two menu pages and their fields — the door, Highlights as references, the title — 24 September 2026

**Where the menu's words live now, all three tiers:** a section's words on its «Разделы меню» record (23 Sep), a dish's on its «Блюда» / «Напитки» post, and — from today — a page's on the page. The author's question opened it: "where do the fields go, «Разделы меню» or pages?" — pages, and "two pages in admin, food and drinks?" There was one: «Menu» (ID 8, `page-menu.php`) rendering both states from `?menu=drinks`. Now there are two.

- **Two pages, the state is the template** (author's preference; the technical case: a page cache never caches a query string, so the bar menu was uncached; each page gets its own `<title>`, canonical, hreflang, revisions and edit lock). `page-menu.php` (Menu) is the kitchen; new `page-menu-bar.php` (Menu — Bar) is the bar; both render one body, `template-parts/menu-page.php`, with the state as its arg — `page-menu.php` lost its 150 lines to it. `inc/menu-page.php` (new): `sweet_pepper_menu_state()` from the queried page's template, `sweet_pepper_menu_page( $state )` found by template (the team may rename the page), `sweet_pepper_menu_url( $state, $section )` — a permalink, so `/en/` rides along. **The bar page: «Меню — бар», slug `bar` under Menu → `/menu/bar/`** (local: page 601, made by the seeder). **`/menu/?menu=drinks` is a 301** to it, in the request's language, the `#section` hash surviving as browsers keep it; until the bar page exists (a fresh site, the test site before its seed) the kitchen template still renders the bar for that URL, so nothing ever 404s. Nine links moved to `sweet_pepper_menu_url()` (home page ×3, hero door, jump-nav, dish picker CTA in PHP and JS, the home hero's party button in `daypart-engine.js`); `inc/daypart-head.php` prints the bar page as night before paint from the template, and the engine reads WordPress's `page-template-page-menu-bar` body class — `?menu=drinks` stays as a checker's switch on any page. The RU connector swap (`sweet_pepper_menu_connector_lang`) asks the page's state, not `$_GET`.
- **`group_sp_menu` «Меню — страница»** — generated in `tools/page-field-groups.py` (prefix `field_sp_mpage_`), located on both templates, three tabs: **«Первый экран»** — the door to the other menu: word ⇄, hover photo (3:2), caption ⇄, paragraph ⇄ (the hero's `__()` door strings are gone; `menu-hero.php` reads `sweet_pepper_menu_door()`); **«Подборка»** — eyebrow ⇄, headline line 1 ⇄, line 2 ⇄, and **a Relationship list of dishes or drinks (max 8, thumbnails in the picker, the same size · price · description label as the section lists')** — a card's name, photo and section link come from the dish's record, never copied (the brief's *surfaces reference dishes*); **«Поиск»** — the page's name in the browser tab ⇄ (WordPress adds the site name; `document_title_parts`) and a meta description ⇄ (`wp_head`) — the first page with a real need for the `<title>` twin the brief listed as not built. What stays in code: the FOOD / DRINKS wordmark, «Ещё блюда / Ещё напитки», the connectors, the default section. The pairing-station heading (`.po`), the Location text and the phone Visit CTA (About's fields, 23 Sep) already had homes — no tab for them.
- **Highlights never blank — three sources in order** (`sweet_pepper_menu_highlights()`): the page's list → while it is empty, **every dish (drink on the bar page) of the menu carrying a seasonal label, in menu order, eight at most** — the "deliberately wrong" option (no field, the label decides) kept as the fallback because the strip is a curated six in a chosen order → while the store has none, the typed cards in `data/menu/page.php` (the old six). **A card's photo is the dish's featured image**, else the photo of the section that places it — the author's concern ("these cards show seasonal dishes, not the section image") answered by the featured image: `dish` / `drink` always supported one; now the box is titled «Фото блюда» / «Фото напитка» with a 3:2 note under it, and the Dishes / Drinks tables open with a «Фото» column so the team sees who has none (nobody yet: 0 of 253). A card links to its section — an anchor on its own page, a URL to the other page. **RU card link = the section's nav label alone, and the RU heading is one line** — the author's 23 Sep decisions found in a parallel session's uncommitted `highlights.php` («Салаты от Перцев» overflows a 262px card), carried into the reader; the strip's heading goes through `sp_headline()` so the empty RU line 2 never borrows «Highlights».
- **Seeded locally** (`tools/page-seed.php menu`; DB backed up first): the bar page created; on both pages the door (photos into the media library), the header, the title and description, and the list = what the empty list shows anyway — 5 seasonal dishes (Летний салат с брынзой, Летний супчик дня, Фетучини Корфу, Жаркое с треской, Равиоли) and 7 seasonal drinks (Ледяная лимонная, Лилиан, Эммануэль, Гемини, Сырный, Сезонный лимонад, Сырный раф) — so the form and the page agree. **Gazpacho and Okroshka**, two of the old six cards, are not in the store (never in `menu.md`), so they left the strip; Caramel cheesecake and Summer salad are in it without a seasonal label — add the label or pick them in «Подборка».
- **Verified** (worktree theme rendered from the CLI against the Local DB, both languages, both pages): against the committed `wip` tree the kitchen page differs only in title, meta, the door's href and the picker CTA's href (plus the cards); the bar page additionally in its own canonical / hreflang / body class / shortlink; `?menu=drinks` → 301 `/menu/bar/`, `/en/menu/?menu=drinks#cocktails` → 301 `/en/menu/bar/`; zero PHP notices; the group loads with 24 fields, the picker's search returns «Фетучини Корфу — 250 г · 465-. · …»; `php -l` and div nesting on every changed template. **Not seen: the admin screens** — no login from the CLI. **Not carried to the main checkout:** it holds ~30 files of another session's uncommitted work, `highlights.php` and `menu-sections.php` among them, so the patch would have clobbered live edits — the author merges this branch instead. Until then the local site's `/menu/bar/` (page 601 exists in the shared DB) renders with `index.php`.
- **Drafts for the author (RU):** the door paragraphs, the door caption «Капучино с мороженым», the eyebrow «вкусно и свежо», both pages' «Поиск» title + description — marked `// draft` in `data/menu/page.php`. EN card links still say "Show in {label}" (the copy doc's "See in" was never propagated).
- **Test site after deploy:** SCF → *Sync available* («Меню — страница»), then `cd ~/repository-test && WP_ROOT=~/SweetPepper-test/public_html php tools/page-seed.php menu` — it creates the bar page there. Then check `/menu/?menu=drinks` redirects.
...
7. **Test site, remaining setup:** seed the menu (`cd ~/repository-test && WP_ROOT=~/SweetPepper-test/public_html php tools/menu-seed.php all`, after SCF *Sync available*), then the menu pages (`… php tools/page-seed.php menu` — creates `/menu/bar/`), remove the dead theme link, delete the four installer plugins, SCF *Sync available* + re-enter field values, permalinks → Post name, **basic auth + discourage search engines before the URL is shared** (the site is public until then), Editor accounts for testers.
...
2. WordPress editability for dish rows — SCF (replaces ACF, 18 Sep 2026). **Decided 23 Sep 2026: the hybrid (Б) — `dish` / `drink` posts placed by `menu_list` Relationship lists; all sections migrated** (entry at the top; `website-brief.md` → *Menu storage*). The list below is the history of the repeater store (А), removed the same day:
   ...
   - Not built yet: ~~hybrid store B for the comparison;~~ ~~the other 14 sections~~ (done 23 Sep); ~~dish photo field~~ (24 Sep: the featured image, «Фото блюда»); the reference dropdown for ~~highlights~~ (24 Sep: «Подборка» on the menu pages) / previews / picker; section-level content: ~~pill, eyebrow, deal card~~ (23 Sep, «Разделы меню») — the section photo + slider still to do.
