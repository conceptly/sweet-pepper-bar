...
|---|---|---|---|
| Event cards — could the VK caption run to two lines without breaking the hover layer? | Home → «Что нового», `events.css` → Title | The author's own ideas on `/?title-lines=2` (kept for this); the hover layer (title + «Смотреть во ВКонтакте») is the constraint — two lines push it above the gradient at 98px. | One line stays unless the hover layer gets its own treatment (a taller body on hover, or the CTA replacing the date instead of stacking). |
