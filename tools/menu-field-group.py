...
# The section's own words (23 Sep 2026, author: the section copy goes into «Разделы меню»):
# read by inc/menu-sections.php → sweet_pepper_menu_sections(); typed fallback and seed
# source data/menu/sections-copy.php. Collapsed — the dish lists are what the team opens for.
SECTION_COPY = [
    ("label", "Название в навигации", "text", "Короткое: меню первого экрана и строка разделов на телефоне"),
    ("cta_label", "Кнопка на первом экране (телефон)", "text", ""),
    ("headline", "Заголовок раздела", "text", "Пусто — как название в навигации"),
    ("eyebrow", "Надзаголовок", "text", ""),
    ("description", "Текст первого экрана", "textarea", ""),
    ("caption", "Подпись фото на первом экране", "text", "Что на фото"),
    ("pill", "Подпись фото раздела", "text", "Что на фото; она же — описание для скринридера"),
    ("alt", "Описание фото для скринридера", "text", "Пусто — как подпись фото раздела"),
    ("deal_title", "Акция: заголовок", "text", "Пусто — карточки акции нет"),
    ("deal_main", "Акция: главная строка", "text", ""),
    ("deal_sub", "Акция: вторая строка", "text", ""),
    ("deal_link", "Акция: ссылка вместо второй строки", "text", "Ведёт в барное меню"),
]
def section_copy_fields(prefix):
    out = [field("sec_more", "Тексты раздела: названия, заголовок, первый экран, фото, акция", "", "accordion",
                 prefix=prefix, open=0, multi_expand=1, endpoint=0)]
    for key, label, kind, note in SECTION_COPY:
        preset = area if kind == "textarea" else text
        for lang, suffix in (("ru", ""), ("en", " (EN)")):
            out.append(field(f"sec_{key}_{lang}", label + suffix, f"sec_{key}_{lang}", kind, 50, prefix=prefix,
                             **{**preset, **({"placeholder": note} if note and kind == "text" else {})}))
    out.append(field("sec_end", "", "", "accordion", prefix=prefix, open=0, multi_expand=0, endpoint=1))
    return out

menu_list = group(
    "group_sp_menu_list", "Раздел меню — списки блюд", [
        *section_copy_fields(L),
