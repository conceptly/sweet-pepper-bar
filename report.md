...
| `cpt.php` | Custom post types (the unused dish taxonomies removed 20 Sep 2026): `dish` (the dishes store under test — not public, Russian labels, revisions; removed if the repeater store wins), **`menu_list`** (20 Sep 2026 — the dishes store's twin of `menu_section`), `news`, and **`menu_section`** (18 Sep 2026 — one record per menu section, not public, revisions on, admin-only creation; listed in menu order) |
| `dish-quick-edit.php` | **20 Sep 2026 — test-only, goes with the dishes store.** Size and price in the Dishes table's Quick Edit: `quick_edit_custom_box` under «Выход и цена», values from `data-*` on a hidden span the column prints, `save_post_dish` (nonce, `edit_post`, `update_field()` by key). |
| `menu-data-dishes.php` | **20 Sep 2026 — test-only.** The dishes store: `sweet_pepper_menu_list_rows( $slug )` (a `menu_list` record's Relationship lists → rows in the repeater's shape), size-and-price labels in the Relationship picker, «Б - Блюда» table columns and A–Z order. Deleted if the repeater store wins |
| `menu-data.php` | **18 Sep 2026.** `sweet_pepper_menu_subsections( $slug )` — a section's subsections → dish-row args in one language, from its `menu_section` record, falling back to `data/menu/<slug>.php`. `sweet_pepper_menu_format_sizes()` builds the price and quantity strings (`260-. / 420-.`, `350 g / 0.5 L`; units twinned RU/EN in `sweet_pepper_menu_units()`). Issues a random stable `dish_id` per row on save; validates two icons at most |
| `pairings.php` | **18 Sep 2026.** `sweet_pepper_food_pairings()` — the six food → drink pairings, **one list for the menu page's pairing station and About's Concept picker** (About carried its own three-dish copy with different wording and one different pairing; the two had drifted). `sweet_pepper_pairing_index( $pairings, $slug )` gives the default pick by slug — both pages open on `roast`. Phase 2: an ACF repeater / dish relationship. |
...
0. **Menu storage — test with the team, then decide.** The author leans **А** (repeater per section, compact rows) after seeing both on the test site: cleaner and more organised than Б's list of every dish. Run the price tasks on both (`testing.md`): reprice a whole section, change one price, hide a dish, undo a wrong edit. When decided, delete the loser: for Б that is the `dish` / `menu_list` types in `inc/cpt.php`, `inc/menu-data-dishes.php`, `inc/dish-quick-edit.php`, two groups in `tools/menu-field-group.py` + `acf-json/`, the `?menu_store=` switch in `inc/menu-data.php` and the `--dishes` branch of `tools/menu-seed.php`. Also confirm the stuck-rail tremble is gone on the phone — record the screen again either way; it can be measured frame by frame (entry of 21 Sep).
1. **Decide on the device, then clean up.** (a) Night bento headline: **Lemon (on trial) or Cream (fallback)** — if Lemon stays, move the Now pip to Lime at night or retire the pip rule; either way remove the "on trial" wording from `hero.css`, the brief and this file. (b) Menu rail entrance + nudge on the iPhone: is the 66% line right, is the nudge tolerable while reading prices. (c) Hero air on the iPad Air / mini in landscape: 32px floor, tiles down to 168 — or a 24px floor with larger tiles. (d) Nav drawer descriptions at 360 (About and Menu still take two lines).
...
3. **Closed state leftovers:** RU copy (drafts in the brief → Closed lines), the Figma `Closed` state, the GOOD NIGHT! window if a night ever ends after 04:00.
...
7. **Test site, remaining setup:** seed Soups **in both stores** (`WP_ROOT=~/SweetPepper-test/public_html php ~/repository-test/tools/menu-seed.php soups`, then the same with `--dishes`; compare at `/menu/#soups` and `/menu/?menu_store=dishes#soups`), remove the dead theme link, delete the four installer plugins, SCF *Sync available* + re-enter field values, permalinks → Post name, **basic auth + discourage search engines before the URL is shared** (the site is public until then), Editor accounts for testers.
...
2. WordPress editability for dish rows — SCF (replaces ACF, 18 Sep 2026); structure narrowed to repeater-with-row-IDs, `dish` CPT, or CPT + Relationship lists — see `website-brief.md` → Content editing → Open → *Menu storage*. The About hero headline field group is the first working example. **Repeater store built for Soups (18 Sep 2026), as the first half of the brief's Soups test:**
   ...
   - Not built yet: ~~hybrid store B for the comparison;~~ the other 14 sections (in either store); dish photo field; the reference dropdown for highlights / previews / picker; section-level content (hero photo, pill, eyebrow, deal card).
