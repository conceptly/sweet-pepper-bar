...
- **Save (⌘S):** the editor re-fetches the file from the server, finds the *n*-th lang-tagged element in the source (scripts, styles and comments masked), checks its tag, language and original text against what the page loaded, replaces only the inner HTML of the changed blocks, and posts the whole file to `style-guide-save.php` with the password and the file's Last-Modified. Any mismatch aborts with a message instead of writing. `<b>`/`<i>` are normalised to `<strong>`/`<em>`.
- **Server (`style-guide-save.php`):** verifies the password against a bcrypt hash in `style-guide-config.php` (git-ignored; template `style-guide-config.example.php`; hash made on the Mac with `htpasswd -nBC 10 x | cut -d: -f2`, so the plain password is stored nowhere), refuses to run without it, rejects payloads that are not the guide or are wildly different in size, returns 409 if the file changed in the instant between the editor's re-fetch and the write (the real concurrency guard is the per-block text check above — verified: a block altered on the server is refused with "differs from the server copy"), copies the current file to `style-guide-backups/` (last 30), then writes atomically. The target file name is fixed in the script; the client never chooses it.
...
- **Consequences:** the server copy of `style-guide.html` is now the source for both languages. `tools/i18n/inject.py` still targets `style-guide-draft.html` only and must never be run against the live file (it would strip the RU layer and rebuild it from the stale JSON). Before any repo-side edit or PDF render, pull the live file first (§3). `design.md` remains the source of rules and numbers — in-page edits are for wording, not for changing a rule.
- **Verified locally (2026-09-08)** against a Python stand-in for the PHP endpoint: block counts match between page and file (674/674, no tag/lang mismatch), a four-block EN+RU save changed exactly those blocks in the file, wrong password rejected, backup written. The PHP itself could not be executed on the Mac (no PHP installed) — first save on the real host is the remaining test.
