...
| **First look, 20 Sep 2026 (test site): the team is fine with either; price is what they change most; they like the clean Dishes table. Same day: А rows compacted to one line + «Подробнее», Б got size/price in Quick Edit — re-run the price tasks on both. Author leans А after seeing the compact rows; the team test decides.** Menu storage: a repeater per section vs. `dish` posts placed by Relationship lists — which admin screen does the team find easier? | Content editing → Open → *Menu storage* (`website-brief.md`) | A manager with the **Editor** role, on Soups, built both ways: (1) change one price, (2) move a dish, (3) reprice all six soups, (4) undo a wrong edit. Watch for hesitation, wrong clicks, and whether the row layout (names · sizes and prices · descriptions) reads at a glance. Repeater side built 18 Sep 2026 («А - Меню Все в одном»); hybrid side built 20 Sep 2026 («Б - Блюда» + «Б - Меню Макет», page at `?menu_store=dishes`). Alternate which variant a participant meets first; the sidebar order (А, Б - Блюда, Б - Меню Макет) stays the same for everyone. Extra things to watch on B: does the manager look for the price in the list screen (it lives on the dish), and does a new dish get placed after it is created. Admin language should be Russian for the session (Settings → General). | Repeater wins (3) clearly and ties (1); both drag for (2); the hybrid's only edge is per-dish history, which revisions on the section record mostly cover. |
