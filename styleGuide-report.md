...
| `style-guide-draft.html` | repo root (~1.5 MB, single file) | Interactive guide, EN + RU in one file with a language switch; app-style mobile layout ≤768px (see §6). Everything inline: base64 Molot, flattened logo SVGs with Copy-SVG buttons, embedded photos/icons/ticket screenshots, tokens in `:root`, print CSS. Sections 00–09 (Notes & open questions removed 2026-09-05 — the guide is audience-facing; open items live in this report and `design.md`). |
| `sweet-pepper-style-guide.pdf` · `-ru.pdf` | repo root | A4 portrait renders of the HTML (EN 27 pp, RU 31 pp). Peppercorn cover, Paper pages, full bleed. |
